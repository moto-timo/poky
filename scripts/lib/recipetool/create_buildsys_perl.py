# Recipe creation tool - create build system handler for perl
#
# Copyright (C) 2015 Yocto Project
#
...
class PythonRecipeHandler(RecipeHandler):
    ...
    # Assumes that the host perl builtin_module_names is sane for target too
    ...
    def process(self, srctree, classes, lines_before, lines_after, handled):
        ...
        if not RecipeHandler.checkfiles(srctree, ['Build.PL']):
            return

        # we need to look at Build.PL or Makefile.PL
        # but perhaps easier to parse Meta.yml or Meta.json these days
        #
        # Build.PL tells us to inherit cpan-build, but if it
        # uses or requires Module::Build we need to fail
        #
        # Makefile.PL tells us to inherit cpan, but we still
        # need to check for 'use Module::Build'
        #
        # Meta.yml and Meta.json can not be entirely trusted to
        # give us all the DEPENDS or RDEPENDS, so we need to
        # scan every file in /lib for 'use ${whatever}'.
        #
        # Also we should look at /t for tests and populate ${PN}-ptest
        # and run-ptest with the packaged tests.
